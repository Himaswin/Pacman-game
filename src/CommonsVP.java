...
public class CommonsVP extends JPanel {
...
// Making  a rotation object
	public static RotationInterpolator ring_Rotator(int speed, TransformGroup inputTG) {
		
//Making transform group to rotate the object
        Transform3D trans3d = new Transform3D();
        trans3d.rotY(Math.PI/2.0);
        inputTG.setTransform(trans3d);
        
//Making the fan rotate in Y axis
        inputTG.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
        Alpha alpha = new Alpha(-1, Alpha.INCREASING_ENABLE, 0, 0, speed, 0, 0, 0, 0, 0);
        RotationInterpolator rotator = new RotationInterpolator(alpha, inputTG, trans3d, 0.0f, (float) Math.PI * 2.0f);
        
        inputTG.addChild(rotator);
		rotator.setSchedulingBounds(hundredBS);
		return rotator;
	}
	...
	public CommonsVP(BranchGroup sceneBG) {
	...
	public static void main(String[] args) {
		frame = new JFrame("VP's Common File");            // NOTE: change XY to student's initials
		frame.getContentPane().add(new CommonsVP(create_Scene()));  // create an instance of the class
